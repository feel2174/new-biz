...
관련 문서: [../skills/Naver_Ad_Keyword_And_Copy_Strategy.md](../skills/Naver_Ad_Keyword_And_Copy_Strategy.md)
— 광고 계정에 등록할 키워드 추출 + 파워링크 광고 소재(제목/설명) 문구 생성 전용.
아래 §7의 키워드 분류 로직을 재사용하니, 새 글을 쓸 때는 이 문서도 함께 참고한다.
...
5. **키워드 리스트를 받았든 안 받았든, 매 페이지마다 광고 키워드를 추출·분류한다** (§7 참고).
   광고 계정 등록 판단 기준은
   [`../skills/Naver_Ad_Keyword_And_Copy_Strategy.md`](../skills/Naver_Ad_Keyword_And_Copy_Strategy.md) 참고
   (소재 문구 작성은 아래 9단계·§12에서 별도로 진행).
...
8. **검증** (§9 체크리스트).
9. **광고 소재(제목/설명) 작성** (§12 참고) — 요청 없어도 매 글마다 빠짐없이 작성한다.
10. **git status/fetch 확인 → 커밋 → push** (§10, 매번 새로 확인. 이전에 확인했다고 넘겨짚지 않는다).
...
- [ ] 광고 소재(제목/설명) 후보 2~3세트 작성 완료 (§12, 요청 없어도 매번)
...
## 12. 광고 소재(제목/설명) 등록용 문구 작성 (2026-07-27 추가)

콘텐츠 페이지를 만들 때마다, 그 페이지로 트래픽을 태울 검색광고(네이버 파워링크,
카카오모먼트, 구글 검색광고 등) 등록에 바로 쓸 수 있는 **소재 문구(제목 + 설명)**도
페이지 본문과 함께 작성한다. 사용자가 매번 별도로 요청하지 않아도 되도록,
글 작성 워크플로우의 표준 산출물로 고정한다 — 사용자가 "앞으로는 글 작성 시 광고
소재 등록(제목/설명)도 함께 작성해달라"고 명시적으로 요청함(2026-07-27, 다른
세션/디바이스에서도 동일하게 적용하라는 지시로 이 문서에 기록).

### 언제 작성하나

- §5 워크플로우 기준 **검증(§9) 다음, git 커밋 전** 단계(step 9)에서 작성한다.
- 예외 없이 **새 글마다 매번** 작성한다 — "이번엔 필요 없다"는 명시적 요청이 없는 한 생략하지 않는다.

### 산출물 형식

- **제목**: 15자 내외. 핵심 키워드 + 검색 의도가 정확히 일치하도록 작성(예: "지뢰찾기 무료 다운로드").
- **설명**: 45자 내외로 2종 이상. 페이지의 핵심 정보 요약 + 행동유도 문구를 담는다.
- 최소 **2~3세트**(제목+설명 조합)를 제공해 광고 등록 시 A/B 테스트가 가능하게 한다.
- 소재 문구는 반드시 **페이지 본문·CTA 목적지와 일치**시킨다. 소재와 랜딩 페이지 내용이
  어긋나면("소재 부정확/과장") 매체 심사에서 반려될 수 있으므로, 페이지에 없는 혜택이나
  키워드를 소재에만 새로 지어내지 않는다.
- 매체(네이버/카카오/구글)별로 글자수 제한과 정책이 조금씩 다르고 수시로 바뀌므로,
  위 "15자/45자"는 **대략적인 가이드일 뿐 절대 기준이 아니다** — 실제 등록 시점에
  해당 매체의 최신 소재 작성 가이드를 다시 확인할 것.

### 어디에 출력하나

- 파일로 별도 저장하지 않고, **글 작성이 끝날 때마다 대화 응답에 "광고 소재 후보"
  섹션으로 바로 제시**한다(제목/설명 세트 나열).
- 사용자가 특정 형식(스프레드시트, 별도 파일 등)으로 저장해달라고 요청하면 그때
  반영한다 — 기본값은 대화 응답 텍스트다.
